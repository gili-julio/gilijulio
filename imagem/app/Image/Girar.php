...
class Girar{
    ...
    private function output($localFile, $quality = 50){
        ...
        switch($this->type){
            case 'jpeg':
                imagejpeg(imagerotate($this->image, 90, 0),$localFile,$quality);
                break;
                
            case 'png':
                imagepng(imagerotate($this->image, 90, imageColorAllocateAlpha($this->image, 0, 0, 0, 127)),$localFile,$quality);
                break;

            case 'bmp':
                imagewbmp(imagerotate($this->image, 270, imageColorAllocateAlpha($this->image, 0, 0, 0, 127)),$localFile,$quality);
                break;

            case 'gif':
                imagegif(imagerotate($this->image, 270, imageColorAllocateAlpha($this->image, 0, 0, 0, 127)),$localFile,$quality);
                break;

            case 'webp':
                imagewebp(imagerotate($this->image, 270, imageColorAllocateAlpha($this->image, 0, 0, 0, 127)),$localFile,$quality);
                break;
