...
if (isset($_POST['submit']) && !empty($_SESSION['rua']) && (!empty($_POST['apartamento'])) or !empty($_SESSION['apartamento'])) {
    ...
    $rua = $_SESSION['rua'];
    if($apartamento == "9999"){
        $sql = "SELECT * FROM produtos WHERE rua = '$rua' GROUP BY codigo_barras ORDER BY descricao";
        $result = $conexao->query($sql);
        unset($apartamento);
        //PARA CONTAR PRODUTOS COM FOTO
        $sqlcomfoto = "SELECT * FROM produtos WHERE rua = '$rua' and nome_foto != '' GROUP BY codigo_barras";
        $resultcomfoto = $conexao->query($sqlcomfoto);
        //CONTAR PRODUTOS TOTAIS
        $sqltotal = "SELECT count(codigo_barras) as total FROM produtos WHERE rua = '$rua' ";
        $resulttotal = $conexao->query($sqltotal);
        $tabelatotal = mysqli_fetch_assoc($resulttotal);
        //CONTAR PRODUTOS ÚNICOS
        $sqldiferentes = "SELECT count(distinct(codigo_barras)) as diferentes FROM produtos WHERE rua = '$rua' ";
        $resultdiferentes = $conexao->query($sqldiferentes);
        $tabeladiferentes = mysqli_fetch_assoc($resultdiferentes);
    } else {
        $sql = "SELECT * FROM produtos WHERE rua = '$rua' and apartamento = '$apartamento' GROUP BY codigo_barras ORDER BY descricao";
        $result = $conexao->query($sql);
        //PARA CONTAR PRODUTOS COM FOTO
        $sqlcomfoto = "SELECT * FROM produtos WHERE rua = '$rua' and apartamento = '$apartamento' and nome_foto != '' GROUP BY codigo_barras";
        $resultcomfoto = $conexao->query($sqlcomfoto);
        //CONTAR PRODUTOS TOTAIS
        $sqltotal = "SELECT count(codigo_barras) as total FROM produtos WHERE rua = '$rua' and apartamento = '$apartamento' ";
        $resulttotal = $conexao->query($sqltotal);
        $tabelatotal = mysqli_fetch_assoc($resulttotal);
        //CONTAR PRODUTOS ÚNICOS
        $sqldiferentes = "SELECT count(distinct(codigo_barras)) as diferentes FROM produtos WHERE rua = '$rua' and apartamento = '$apartamento' ";
        $resultdiferentes = $conexao->query($sqldiferentes);
        $tabeladiferentes = mysqli_fetch_assoc($resultdiferentes);
    }
...
<head>
    ...
    <title> Rua <?php echo $rua; ?> <?php if(isset($apartamento)){ echo "> A".$apartamento; }?></title>
    <style>
        .cimainicio {
...
<body class="cimainicio">
    <div class="container">
        ...
        <div class="bg text-center container col">
            <h1><?php if(isset($apartamento)){ echo "Apartamento ".$apartamento." da"; }?> Rua <?php echo $rua; ?></h1>
            <h3>(Total de produtos: <strong><?php echo mysqli_num_rows($result);?></strong>)</h3>
            <h5><?php echo mysqli_num_rows($resultcomfoto);?> com foto || <?php echo ((mysqli_num_rows($result))-(mysqli_num_rows($resultcomfoto)));?> sem foto</h5>
            <h6><?php if($tabelatotal['total']-$tabeladiferentes['diferentes'] != 0){ if($tabelatotal['total']-$tabeladiferentes['diferentes'] == 1){ echo "1 repetição não exibida"; } else {echo ($tabelatotal['total']-$tabeladiferentes['diferentes'])." repetições não exibidas";} }?></h6>
            <br>
                    ...
                    <tbody class="">
                        ...
                        <?php
                            while($tabela = mysqli_fetch_assoc($result)){
                                ...
                                /*echo "<td class='text-center borda'>"."<a class='blogin' href='#'>
                                        <svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-eye-fill' viewBox='0 0 16 16'>
                                          <path d='M10.5 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0z'/>
                                          <path d='M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8zm8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z'/>
                                        </svg>
