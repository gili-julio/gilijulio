...
if(($_SESSION['usuario'] == 'avaria') && ($_SESSION['senha'] == 'avaria')){
    ...
    //SQL responsável por buscar os títulos das tabelas
    $sqlavaria = "SELECT * FROM avaria_bolachas WHERE quantidade = 'null' ";
    ...
    //VARIAVEL PARA DETERMINAR EDIÇÃO DE ALGUM ITEM
        ...
        if($editar == 'sim'){
            ...
            $destino = $_SESSION['destino_avaria'];
        } else {
            $_SESSION['barras_avaria'] = 'nada';
            $_SESSION['descricao_avaria'] = 'nada';
            $_SESSION['qtd_avaria'] = 'nada';
            $_SESSION['destino_avaria'] = 'nada';
        }
        
        
    //AO CLICAR NO BOTÃO CRIAR
    if(isset($_POST['submit'])){
        //VERIFICA SE EXISTE NOME DO FORNECEDOR E COR PARA TABELA
        if(isset($_POST['nome']) && isset($_POST['cor'])){
            ...
            //TESTA SE HÁ UMA COR SELECIONADA
            if($valorcor == "999"){
                $_SESSION['erro'] = "cor";
                unset($_POST);
                ...
                //MUDA A COR DE ACORDO COM O CASO
                switch($valorcor){
                    case '1':
                        $cor = 'Vermelho';
                        break;
                    ...
                    //TESTA SE HÁ ESSA TABELA
                        $sqlteste = "SELECT * FROM avaria_bolachas WHERE nome = '$fornecedor'";
                        $resultteste = $conexao->query($sqlteste);
                    if(mysqli_num_rows($resultteste) == 0){
                    //NÂO EXISTE TABELA DESSE FORNECEDOR
                        //CRIA A TABELA
                            $sqladd = "INSERT INTO `avaria_bolachas` (`nome`, `quantidade`, `identificador`, `cor`) VALUES ('$fornecedor', 'null', '$fornecedor', '$cor')";
                            $resultadd = $conexao->query($sqladd);
                        $_SESSION['erro'] = "sucesso";
                        unset($_POST);
                        header('Location: avariaBolachas.php');
                    } else {
                    //EXISTE TABELA DESSE FORNECEDOR        
...
<!-- fim do preloader --> 
    ...
    <br><br>
        ...
        if ($erro == "cor") {
            echo "<div class='alert alert-info col-4 offset-4 alert-dismissible fade show text-center' role='alert'>
                    <strong><h3>Cor não selecionada</h3></strong>
                    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                </div>";
        }
        if ($erro == "existe") {
            echo "<div class='alert alert-info col-4 offset-4 alert-dismissible fade show text-center' role='alert'>
                    <strong><h3>Já existe esse fornecedor</h3></strong>
                    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                </div>";
        }
        ?>
    <div class="text-center container">
        <table class="table text-start">
                    <tbody class="">
                        <tr class="borda">
                            <form action="avariaBolachas.php" method="POST">
                                <?php
                                    if($editar == 'nao'){
                                        echo "<th colspan='1'>
                                            <input type='text' class='form-control text-start' name='nome' autocomplete='off' placeholder='Nome do Fornecedor'>
                                        </th>
                                        <th>
                                            <select class='form-select text-start' id='Select' name='cor' >
                                                    <option value='999'>Selecione uma cor</option>
                                        ...
                                        <th class='text-end'>
                                            <input class='bgreen' type='submit' name='submit' id='submit' value='Criar'>
                                        </th>";
                                    } else {
                                        echo "<th>
                                        ...
                                        <th>
                                            <select class='form-select text-start' id='Select' name='destino'>
                                                    ";
                                                    switch($destino){
                                                        case '1':
                                                            echo "  
                        ...
                        <tr>
                            <?php 
                                if(mysqli_num_rows($resultavaria) == 0){
                                    ...
                                    echo "<th colspan='3'>".$linhas." fornecedores</th> ";
                                    echo "</tr>";
                                    echo "<tr class='text-center align-middle'>";
                                    echo "<th class='text-start'> <strong>Fornecedor</strong> </th>";
                                    echo "<th> <strong>Produtos</strong> </th>";
                                    echo "<th> <a class='bred d-inline-block' href=''>Limpar</a></th>";
                                    echo "</tr>";
                                    while($tabelaavaria = mysqli_fetch_assoc($resultavaria)){
                                        $fornecedor = $tabelaavaria['nome'];
                                        $sqlfor = "SELECT * FROM avaria_bolachas WHERE identificador = '$fornecedor'";
                                        $resultfor = $conexao->query($sqlfor);
                                        $produtos = mysqli_num_rows($resultfor)-1;
                                        echo "<tr>";
                                        echo "<th>".$tabelaavaria['nome']."</th>";
                                        echo "<th class='text-center'>".$produtos." produtos </th>";
                                        echo "<th class='text-center'>
                                        <a class='bgreen d-inline-block' href='subAvariaBolachas.php?fornecedor=$fornecedor'>
                                        Abrir
                                        </a>
                                        <a class='bred d-inline-block' href='excluirBolachas.php?excluirfornecedor=$fornecedor'>
    ...
    <?php
        if(mysqli_num_rows($resultavaria) != 0){
            echo "<div class='text-center container'>
                <a href='' class='bgreen d-inline-block'>Gerar arquivo com tudo</a>
            </div>";
        }
    ?>
    <br><br>
