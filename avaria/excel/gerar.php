...
//DADOS INICIAIS E TEMPORAL
    $hoje = date('d/m/Y');
    $hoje2 = date('d_m_Y');
    $sheet->setCellValue('A1', 'CONTROLE DE AVARIAS - '.$hoje);
//PARA SALVAR O NOME DO ARQUIVO
    // CRIA UMA VARIAVEL E ARMAZENA A HORA ATUAL DO FUSO-HORÀRIO DEFINIDO (BRASÍLIA)
        $dataLocal = date('H_i_s', time());
        $nome = 'Avaria Dia '.$hoje2.' Hora '.$dataLocal;

//INSERIR OS DADOS DA TABELA AVARIA
    $sqlbusca = "SELECT * FROM avaria_loja";
    ...
    while($tabela = mysqli_fetch_assoc($resultbusca)){
        ...
        $sheet->setCellValue('D'.$contador, $tabela['motivo']);
        $contador = $contador+1;
    }
