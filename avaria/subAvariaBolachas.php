...
if(!empty($_SESSION['usuario']) && !empty($_SESSION['senha'])){
    ...
    if(($_SESSION['usuario'] == 'avaria') && ($_SESSION['senha'] == 'avaria')){
        ...
        //TESTAR SE HÁ GET FORNECEDOR
        if(!empty($_GET['fornecedor'])){
            
            //SQL responsável por buscar a tabela
            $fornecedor = $_GET['fornecedor'];
            $sqlavaria = "SELECT * FROM avaria_bolachas WHERE quantidade != 'null' and identificador = '$fornecedor' ";
            ...
            if(isset($_POST['submit'])){
                //VERIFICA SE EXISTE NOME DO FORNECEDOR E QUANTIDADE PARA TABELA
                if(isset($_POST['nome']) && isset($_POST['quantidade'])){
                    ...
                    if($editar == 'sim'){
                        ...
                        $_SESSION['editar'] = 'nao';
                            ...
                            //TESTA SE HÁ ESSE PRODUTO
                                $sqlteste = "SELECT * FROM avaria_bolachas WHERE identificador = '$fornecedor' and nome = '$produto'";
                                $resultteste = $conexao->query($sqlteste);
                            if(mysqli_num_rows($resultteste) == 0){
                            //NÂO EXISTE ESSE PRODUTO
                                //ADICIONA O PRODUTO
                                    $sqladd = "INSERT INTO `avaria_bolachas` (`nome`, `quantidade`, `identificador`, `cor`) VALUES ('$produto', '$quantidade', '$fornecedor', 'null')";
                                    $resultadd = $conexao->query($sqladd);
                                $_SESSION['erro'] = "sucesso";
                                unset($_POST);
                                header('Location: subAvariaBolachas.php?fornecedor='.$fornecedor);
                            } else {
                            //EXISTE ESSE PRODUTO       
            ...
            //ELSE DO GET FORNECEDOR
            header('Location: avariaBolachas.php');
        }
    
    } else {
        //ELSE DO TESTAR LOGIN CERTO
...
<head>
    ...
    <title><?php echo $fornecedor; ?></title>
</head>
...
<!-- fim do preloader --> 
    ...
    <div class="w-100 container">
        ...
        <div class='d-inline-block col-12 col-lg-3 offset-lg-9 text-white text-end' >
            <a href="avariaBolachas.php" class="bred d-inline-block">
                Voltar
            </a>
        </div>
    </div>
    <br><br>
        ...
        if ($erro == "existe") {
            echo "<div class='alert alert-info col-4 offset-4 alert-dismissible fade show text-center' role='alert'>
                    <strong><h3>Produto já existe</h3></strong>
                    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                </div>";
        }
        ?>
    <div class="text-center container">
        <table class="table text-start">
                    <tbody class="">
                        <tr class="borda">
                            <form action="subAvariaBolachas.php?fornecedor=<?php echo $fornecedor;?>" method="POST">
                                <?php
                                    if($editar == 'nao'){
                                        echo "<th colspan='1'>
                                            <input type='text' class='form-control text-start' name='nome' autocomplete='off' placeholder='Nome do Produto'>
                                        </th>
                                        echo <th colspan='1'>
                                            <input type='text' class='form-control text-start' name='quantidade' autocomplete='off' placeholder='Quantidade'>
                                        </th>
                                        <th class='text-end'>
                                        ...
                                        echo "<th>
                                            <input type='number' class='form-control text-center' name='nome' autocomplete='off' placeholder='Nome do Produto' value='$produto'>
                                        </th>
                                        <th>
                        ...
                        <tr>
                            <?php 
                                if(mysqli_num_rows($resultavaria) == 0){
                                    echo "<tr><th class='text-center' colspan='5'> Sem produtos</th> </tr>";
                                } else {
                                    $linhas = mysqli_num_rows($resultavaria);
                                    echo "<tr class='text-center align-middle'>";
                                    echo "<th colspan='3'>".$linhas." produtos</th> ";
                                    echo "</tr>";
                                    echo "<tr class='text-center align-middle'>";
                                    echo "<th class='text-start'> <strong>Produto</strong> </th>";
                                    ...
                                    while($tabelaavaria = mysqli_fetch_assoc($resultavaria)){
                                        ...
                                        <a class='bred d-inline-block' href=''>
    ...
    <?php
        if(mysqli_num_rows($resultavaria) != 0){
            echo "<div class='text-center container'>
                <a href='' class='bgreen d-inline-block'>Gerar arquivo tabela ".$fornecedor."</a>
            </div>";
        }
    ?>
    <br><br>
